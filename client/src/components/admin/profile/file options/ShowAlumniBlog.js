...
import { fetchAllAlumniBlogsByAdmin} from '../../../actions/admin/index';
...
class ShowAlumniBlog extends Component {
    ...
    componentDidMount() {
        this.props.fetchAllAlumniBlogsByAdmin(this.state.page);
    }
        ...
        if (this.props.Blogs){
           return(<div onScroll={this.roll}  ref={this.userblog} className="ShowUserBlog">{
            this.props.Blogs?this.props.Blogs.map((blog)=>{
                return(<Link to={`/admin/alumniBlog/${blog._id}`}><div className="ShowUserBlog-EachBlog">
                    <h4>
                        {blog.title}
                    </h4>
...
const mapStateToProps = (state) => {
    if(state.Blogs.AlumniBlogs===undefined){
        return {Blogs:[]}
    }
    return { Blogs:Object.values(state.Blogs.AlumniBlogs)};
}

export default connect(mapStateToProps, {fetchAllAlumniBlogsByAdmin})(ShowAlumniBlog);