...
import { UploadOwnProfilePic, UserOwnProfilePic } from '../../../actions/user/index'
...
class Image extends Component {
    ...
    uploadFile = () => {
        ...
        this.props.UploadOwnProfilePic(fd);
    }
    render() {
        if (this.props.profile) {
            let image=new Buffer.from(this.props.profile.pic.data).toString('base64');
...
const mapStateToProps = (state) => {
    return ({ profile: state.UserProfile.profilePic})
}

export default connect(mapStateToProps, { UploadOwnProfilePic, UserOwnProfilePic })(Image);